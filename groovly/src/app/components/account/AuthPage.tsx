        ...
        <div className="relative w-full max-w-[520px]">
          ...
          {status === "success" ? (
            <div className="tab-enter rounded-3xl p-7 sm:p-9 text-center" style={{ background: PANEL, border: `1px solid ${NEON}30`, boxShadow: `0 24px 80px ${NEON}0D` }}>
              ...
              <h3 className="text-2xl font-black" style={{ fontFamily: "Outfit, sans-serif" }}>{mode === "login" ? "Login preview complete" : "Account setup preview ready"}</h3>
              <p className="mt-3 text-sm leading-relaxed" style={{ color: MUTED }}>{mode === "login" ? "No session was created. Connect a production identity provider before enabling real sign-in." : "No account was created and your password was not stored. This preview only shows the intended setup flow."}</p>
            ...
            <form onSubmit={submit} className="rounded-3xl p-5 sm:p-7" style={{ background: PANEL, border: "1px solid rgba(255,255,255,0.08)", boxShadow: "0 28px 90px rgba(0,0,0,0.45)" }}>
              <div className="grid grid-cols-2 rounded-xl p-1 mb-6" style={{ background: INPUT }}>
                {(["login", "signup"] as AuthMode[]).map(item => <button key={item} type="button" onClick={() => { setMode(item); setRole(item === "signup" ? "teacher" : "student"); setStatus("idle"); setError(""); }} className="min-h-11 rounded-lg text-sm font-bold transition-colors" style={mode === item ? { background: "#181824", color: "#fff" } : { color: "#77778B" }}>{item === "login" ? "Log in" : "Sign up"}</button>)}
              </div>
